...
<div class="relative overflow-x-auto w-[85%] sm:rounded-lg mx-auto mt-9">
    @if ($isFiltered)
        <a href="{{ route('index') }}" class="btn btn-secondary">Показать все посты</a>
    @else
        <form action="{{ route('index') }}" method="GET">
            <button type="submit" name="filter" value="my_posts" class="btn btn-primary">
...
{{--                @if($isAdmin || ($articles->user_id == auth()->user()->id))--}}
                @can('update', $articles)
